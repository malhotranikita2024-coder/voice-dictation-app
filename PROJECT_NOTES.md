...
**Release.** Tagged and published `v0.1.0` on GitHub with `dist/Ramble-Setup.exe` as the release asset. Stable download link: `https://github.com/malhotranikita2024-coder/voice-dictation-app/releases/latest/download/Ramble-Setup.exe` — this is what Terminal 10's landing page button should point at, since it always resolves to whatever the latest release's same-named asset is, without needing the landing page edited on every future version bump.
