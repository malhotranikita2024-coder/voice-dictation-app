...
### Taskbar icon shows generic Electron logo on the installed app — likely Windows icon cache, unresolved
- **Observed during:** v3, first real install of `v0.1.0`
- **Symptom:** The running app's taskbar entry shows Electron's default icon, not Ramble's branded one.
- **Investigated and ruled out as a code bug:** every taskbar-eligible `BrowserWindow` (`settingsWindow`, `onboardingWindow`) already sets a real, correct `icon:` option; `mainWindow` and the pill window are both `skipTaskbar: true` by design and never show in the taskbar at all; `build.win.icon` correctly points at the real multi-resolution `assets/icon.ico` added earlier this session.
- **Best-supported hypothesis:** Windows' shell icon cache, keyed by the app's `AppUserModelID` (`com.ramble.app`, set in `main.js:28` since v2). Every dev-mode run and every sandbox test launch of `dist/win-unpacked/Ramble.exe` this session registered activity under that same AUMID — including ones from before `assets/icon.ico` existed — and Windows can keep serving whatever icon it first cached for that AUMID regardless of what the currently-installed `.exe` actually embeds.
- **Real fix:** needs manual verification on a real desktop, which this sandbox cannot do (no interactive desktop session — see the Terminal 9 entry above): uninstall, clear Windows' icon cache (`ie4uinit.exe -ClearIconCache` + restart Explorer, or reboot), reinstall fresh, recheck. If the icon is still wrong after a genuinely clean cache, that's a real remaining bug and needs fresh investigation — nothing found so far points at one.
