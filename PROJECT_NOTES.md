...
| 2026-08-31 | Added explicit `files` exclusions (`!.env`, `!.git`, `!.claude`, `!dist`, `!PROJECT_NOTES.md`, `!README.md`) to `package.json`'s electron-builder config | electron-builder's default file selection does **not** read `.gitignore` — without an explicit `files` list, `.env` (real API keys) would ship inside the packaged app. Confirmed via `npx asar list` on the built `app.asar` that none of the excluded files end up inside. |
| 2026-08-31 | Checked in a real `assets/icon.ico` (built once from `tray-icon-source.png` via electron-builder's own converter) instead of leaving `build.win.icon` pointed at the source PNG | Keeps the exact multi-resolution icon reviewable in git and stable across machines, rather than silently regenerated by whichever electron-builder version happens to run the build. |
| 2026-08-31 | Set `artifactName` to the version-free `${productName}-Setup.exe`, and bumped `package.json`'s `version` from the stale `1.0.0` to `0.1.0` | A versioned artifact filename would break the GitHub Releases `/releases/latest/download/<filename>` link on every future release bump — the landing page (Terminal 10) needs one link that never changes. `0.1.0` also just reflects this build's actual maturity — `1.0.0` was never a deliberate choice, it was `npm init`'s default that nothing had touched since. |
...
**Session 1 — bundle to .exe + GitHub Release.**

Terminal 9's job was the jump from "an app I run with `npm start`" to "a file a stranger can download and double-click." Most of the actual packaging config was small — electron-builder already had a `build` key from earlier ad-hoc testing (a `dist/win-unpacked` folder from a prior `--dir` run was sitting in the repo already) — but two things needed real attention rather than defaults.

The first was `.env` safety. It would be easy to assume electron-builder respects `.gitignore` the way most tooling does — it doesn't. Its default file selection only strips a short hardcoded list of junk (`.git`, `.DS_Store`, lockfiles, and the like); `.env` isn't on it. Without an explicit `files` array, the real Deepgram/Groq keys sitting in the dev `.env` would have been packaged straight into `app.asar` and shipped to anyone who downloaded the installer. Added an explicit `files` list excluding `.env`, `.git`, `.claude`, `dist`, `PROJECT_NOTES.md`, and `README.md`, then verified it actually worked by running `npx asar list` against the built `app.asar` and confirming none of those paths appear (the only `README`-shaped matches left are inside `node_modules` dependencies, which is expected and harmless). This is exactly the kind of gap `secrets-handling`'s checklist exists to catch, but it's worth flagging here too since it's specific to the build step, not commit hygiene.

The second was the icon. `build.win.icon` had been pointing at the raw `tray-icon-source.png` — electron-builder converts that to an `.ico` at build time, which works, but means the actual icon artifact isn't reviewable or stable across builds. A prior test build had already produced a correctly-converted `dist/.icon-ico/icon.ico` from that same PNG, so rather than generating a new placeholder from scratch, that real icon got copied into `assets/icon.ico` and `build.win.icon` now points there directly.

Also renamed the release artifact via `artifactName: "${productName}-Setup.exe"` (electron-builder's default embeds the version number in the filename, which would silently break a `/releases/latest/download/Ramble-Setup.exe`-style stable link every time the version bumps), and bumped `package.json`'s `version` from the untouched `npm init` default of `1.0.0` down to `0.1.0` to match the `v0.1.0` release tag and to stop implying a 1.0 maturity level the app hasn't reached.
...
### Packaged app reused dev's config on first install — FIXED (v3, `extraMetadata.name`)
- **Observed during:** v3, first real install of `v0.1.0` on Nikita's own machine
- **Symptom:** Opening the freshly-installed app from the tray showed his existing dev-mode Settings (real saved API keys, hotkey) instead of a blank profile, and onboarding never appeared on first launch.
- **Cause:** `app.getName()` defaults to package.json's top-level `name` (`voice-dictation-app`), which dev (`npm start`) and the packaged build shared with no override — so both resolved to the same `%APPDATA%\voice-dictation-app\config.json` that `electron-store` reads/writes. See the 2026-08-31 decision log entry above for the full mechanism (confirmed via reading `electron-store`'s source: the `Store` constructor resolves `app.getPath('userData')` synchronously, before almost anything else in `main.js` even runs).
- **Fix:** `build.extraMetadata.name: "ramble"` in `package.json` — packaged app now starts fresh at `%APPDATA%\ramble\`. Verified with the same Playwright-driven `_electron` technique from the Terminal 9 sandbox-testing entry above: launched the rebuilt `dist/win-unpacked/Ramble.exe` with a brand-new isolated `--user-data-dir`, confirmed the onboarding window now appears (screenshot taken). Not yet verified on Nikita's real desktop with the actual installer.

### Taskbar icon shows generic Electron logo on the installed app — FIXED (icon.ico for window icons, not a cache issue)
- **Observed during:** v3, first real install of `v0.1.0`
- **Symptom:** The running app's taskbar entry (the live button shown while Settings/Onboarding is open — confirmed with Nikita this is specifically what was wrong, not a pinned shortcut or the tray icon) shows Electron's default icon, not Ramble's branded one.
- **First hypothesis (WRONG, ruled out by direct test):** suspected Windows' shell icon cache, keyed by the shared `AppUserModelID`. Nikita did the full clean-cache test — uninstall, `ie4uinit.exe -ClearIconCache`, restart Explorer, reinstall — and the icon was still wrong. Not a cache issue.
- **Actual root cause, found by extracting and comparing the real assets:** `[System.Drawing.Icon]::ExtractAssociatedIcon()` on the built `.exe` confirmed its embedded icon resource *is* correct (the real mic icon) — `build.win.icon` → `assets/icon.ico` works fine, so the `.exe` itself and the system tray (`assets/tray-icon.png`, small and pre-sized) were never the problem. The actual difference: every `BrowserWindow`'s `icon:` option (`mainWindow`, `settingsWindow`, `onboardingWindow`, the pill window) was pointed at `assets/tray-icon-source.png` — confirmed via `System.Drawing.Image` to be **1254×1254px, 1.1MB, no alpha channel** — the raw unprocessed source image everything else was generated *from*, not a proper icon asset. Windows' window-icon path (`WM_SETICON`, backing a live taskbar button) is a pickier, different code path than the tray icon or the `.exe` resource, both of which already used small, purpose-built assets.
- **Fix:** pointed all four `BrowserWindow` `icon:` options at `assets/icon.ico` instead — the same multi-resolution icon file already proven correct for the `.exe` itself (`main.js:85,110,150`, `src/main/pill.js:32`). Rebuilt and confirmed the app still launches cleanly with the new icon path (no error), verified via Playwright + Electron's own `BrowserWindow` API that windows still construct correctly. The actual visual taskbar rendering still needs Nikita's real desktop to confirm — this sandbox has no interactive desktop session (see the Terminal 9 entry above) so a live taskbar button can't be visually inspected from here.

### Settings/Onboarding windows had no minimize button — FIXED
- **Observed during:** v3, after installing the fresh-config fix build
- **Symptom:** Neither window could be minimized.
